...
export class AppSummaryView {
  ...
  private selectedMap: Map<string, string[]> = new Map<string, string[]>();
    ...
    this.empService.getEmployeeGithubDetails()
      .subscribe(repos => {
        for (let repo of repos) {
          this.githubRepos.push({ name: repo.name, technology: repo.language, repoUrl: repo.html_url, lastUpdated: repo.updated_at });
        }
